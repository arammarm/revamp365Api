...
class IncrementalInvestorETL
{
    private $mysqlConn;
    private $pgConn;
    private $batchSize = 5000;  // Smaller batches for incremental
    ...
    private function getLastRunState()
    {
        if (!file_exists($this->stateFile)) {
            // First run - process last 90 days only
            return [
                'last_run_time' => date('Y-m-d', strtotime('-90 days')),
                'last_property_id' => 0,
                'total_processed' => 0
            ];
        }
    ...
    public function run()
    {
        try {
            ...
            $state['last_run_time'] = '2009-01-01';
            ...
            // Step 2: Update market activity for active areas
            $this->updateMarketActivity($state['last_run_time']);

            // Step 3: Update search index for changed entities
            $this->updateSearchIndex($state['last_run_time']);

            // Step 4: Update investor sales data
            $this->updateInvestorSalesData($state['last_run_time']);
    ...
    private function processChangedInvestors($lastRunTime, $startTime)
    {
        ...
        // Get investors who made recent purchases and sales (combined with proper pagination)
        $sql = "
            SELECT * FROM (
                ...
                SELECT
                    ...
                    MIN(CAST(dp.CurrentSaleRecordingDate AS DATE)) AS first_purchase,
                    MAX(CAST(dp.CurrentSaleRecordingDate AS DATE)) AS last_purchase,
                ...
                WHERE
                  ...
                  AND DATE(dp.CurrentSaleRecordingDate) >= :last_run_time
                ...
                GROUP BY
                    ...
                    dp.MailingState,
                    dp.MailingZIP5
                HAVING properties_total >= 2
                
                UNION ALL
                
                -- Get investors who made recent sales (as sellers)
                SELECT
                    CONCAT(
                      COALESCE(dp.MailingFullStreetAddress, ''), ' ',
                      COALESCE(dp.MailingCity, ''), ' ',
                    ...
                    MAX(CASE
                        ...
                        END) AS is_corporate,
                    COUNT(DISTINCT dp.PropertyID) AS properties_total,
                    0 AS properties_absentee, -- We don't know occupancy for sales
                    0 AS cash_purchases, -- Not applicable for sales
                    SUM(dp.CurrentSalesPrice) AS total_investment,
                    AVG(dp.CurrentSalesPrice) AS avg_property_value,
                    MIN(CAST(dp.CurrentSaleRecordingDate AS DATE)) AS first_purchase,
                    MAX(CAST(dp.CurrentSaleRecordingDate AS DATE)) AS last_purchase,
                ...
                WHERE
                  ...
                  AND dp.CurrentSaleRecordingDate REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
                  AND (
                      UPPER(dp.CurrentSaleSeller1FullName) REGEXP 'LLC|INC|CORP|TRUST|PROPERTIES|CAPITAL'
                      OR dp.CurrentSaleSeller1FullName IN (
                          SELECT DISTINCT dt2.OwnerNAME1FULL
        ...
        $upsertStmt = $this->pgConn->prepare("
            INSERT INTO investor_cache.investors (
                ...
                properties_owned_total, properties_owned_absentee,
                properties_cash_purchased, total_investment, avg_property_value,
                first_purchase_date, last_purchase_date, days_inactive,
          ...
          ON CONFLICT (mailing_key) DO UPDATE SET
                entity_names = CASE 
                    WHEN EXCLUDED.entity_names IS NOT NULL AND EXCLUDED.entity_names != '{}' THEN
                        CASE 
                            WHEN investors.entity_names IS NULL OR investors.entity_names = '{}' THEN EXCLUDED.entity_names
                            ELSE array(SELECT DISTINCT unnest(investors.entity_names || EXCLUDED.entity_names) ORDER BY 1)
                        END
                    ELSE investors.entity_names
                END,
                properties_owned_total = EXCLUDED.properties_owned_total,
                properties_owned_absentee = EXCLUDED.properties_owned_absentee,
                properties_cash_purchased = EXCLUDED.properties_cash_purchased,
                total_investment = EXCLUDED.total_investment,
                avg_property_value = EXCLUDED.avg_property_value,
        ...
        do { //  Each batch is processed one by one
            ...
            if ($this->logging) {
                $this->printLogging("Processing: Batch " . $batchIndex . " (" . $totalProcessed . " - " . ($totalProcessed + count($rows)) . ") investors total time taken: " . round(microtime(true) - $startTime, 2) . "s");
            }
            ...
            foreach ($rows as $row) {
                ...
                if ($result['count'] > 0) {
                    print (PHP_EOL . "Skipping: " . $mailingKey . " - already has " . $activityType . " records");
                    continue;
                }


                if ($this->logging) {
                    print (PHP_EOL . "Processing: " . $rowIndex . " of " . $rowCount . " - " . $row['mailing_key']);
                }
                ...
                try {
                    ...
                    $lastPurchase = $row['last_purchase'] ? new DateTime($row['last_purchase']) : null;
                    $daysInactive = $lastPurchase ?
                        $lastPurchase->diff(new DateTime())->days : 999999;
                    ...
                    $upsertStmt->execute([
                        ...
                        'properties_owned_total' => $row['properties_total'],
                        'properties_owned_absentee' => $row['properties_absentee'],
                        'properties_cash_purchased' => $row['cash_purchases'],
                        'total_investment' => $row['total_investment'],
                        'avg_property_value' => $row['avg_property_value'],
                        'first_purchase_date' => $row['first_purchase'],
                        'last_purchase_date' => $row['last_purchase'],
                        'days_inactive' => $daysInactive,
                        'last_activity_date' => $row['last_purchase'],
                    ]);
            ...
            // try {
            //     $this->updateInvestorSalesData($lastRunTime);
            // } catch (Exception $e) {
            //     $this->log("Error updating investor sales data: " . $e->getMessage(), 'ERROR');
            // }
    ...
    {
        // Get current property IDs that this investor owns
        $sql = "
            SELECT PropertyID 
        ...
        $currentPropertyIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

        // Mark properties as not owned if they're not in current ownership list
        if (!empty($currentPropertyIds)) {
            $placeholders = implode(',', array_fill(0, count($currentPropertyIds), '?'));
            $updateStmt = $this->pgConn->prepare("
                UPDATE investor_cache.properties 
                SET currently_owned = FALSE, sale_date = NOW()::date
                WHERE investor_id = ?
                    AND datatree_id NOT IN ($placeholders)
                    AND currently_owned = TRUE
            ");

            $params = array_merge([$investorId], $currentPropertyIds);
            $updateStmt->execute($params);
        }

        // UPSERT current properties (recent purchases)
        $sql = "
            SELECT 
                PropertyID, APN, FIPS,
        ...
        $propUpsert = $this->pgConn->prepare("
            INSERT INTO investor_cache.properties (
                ...
                cash_purchase, currently_owned, is_absentee, activity_type
            ) VALUES (
                :investor_id, :datatree_id, :apn, :fips,
                :situs_address, :situs_city, :situs_state, :situs_zip5,
    ...
    private function processSaleProperties($investorId, $investorData, $lastRunTime, $statesInQuery)
    {
        ...
        $propUpsert = $this->pgConn->prepare("
            INSERT INTO investor_cache.properties (
                ...
                location, property_class, bedrooms, bathrooms,
                sqft, year_built, purchase_date, purchase_price,
                sale_date, sale_price, cash_purchase, currently_owned, is_absentee, activity_type
            ) VALUES (
                :investor_id, :datatree_id, :apn, :fips,
                :situs_address, :situs_city, :situs_state, :situs_zip5,
                ST_MakePoint(:longitude, :latitude), :property_class, :bedrooms, :bathrooms,
     ...
     */
    private function updateInvestorSalesData($lastRunTime)
    {
