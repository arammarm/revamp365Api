...
// // Handle county_info - it can be a comma-separated string or an array
// if (isset($filters['county_info'])) {
//     if (is_string($filters['county_info']) && !empty($filters['county_info'])) {
//         // Handle comma-separated string format: "80,Delaware,PA,42045,Delaware, PA,delaware-pa"
//         $countyParts = explode(',', $filters['county_info']);
//         if (count($countyParts) >= 4) {
//             $filters['county'] = trim($countyParts[3]); // FIPS is at index 3
//         }
//     } elseif (is_array($filters['county_info'])) {
//         // Handle array format
//         if (isset($filters['county_info']['fips'])) {
//             $filters['county'] = $filters['county_info']['fips'];
//         }
//     }
//     unset($filters['county_info']);
// }
...
if (empty($filters['county']) && empty($filters['property_ids'])) {
    return APIHelper::SendResponse(null, 0, 'County FIPS is required when property_ids is not provided');
}
...
try {
    ...
    foreach ($results as $result) {
        $transformedResults[] = DatatreeAPIHelper::transformPropertyData($result);
    }

    // Return results
    return APIHelper::SendResponse($transformedResults, 1, 'Results found: ' . count($transformedResults));

} catch (Exception $e) {
    return APIHelper::SendResponse(null, 0, 'Database error: ' . $e->getMessage());
}